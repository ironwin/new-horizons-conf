...
| :--- | :--- | :--- | :--- | :--- |
| **월 ~ 목** | 07:00 ~ 08:00 (기상/출근) | *(슬라이드쇼 OFF)* | `config.js.base` | 시계, 날씨, 캘린더, 알림 등 기본 정보에 집중 |
| | 08:00 ~ 19:xx (낮 시간) | *(시스템 전원 종료)* | - | 08:00 자동 셧다운 (외출 시간 전력 절감) |
| | **20:00 ~ 21:55 (야간)** | **`MMM-OnThisDaySlideshow`** | `config.js.onthisdayslideshow` | **과거의 오늘 사진** 재생 ➔ 완료 시 지정 앨범 슬라이드쇼 전환 |
| | 21:55 ~ 익일 07:00 | *(시스템 전원 종료)* | - | 21:55 야간 자동 셧다운 (스마트 플러그 차단 대비) |
| **금요일** | 07:00 ~ 08:00 | *(슬라이드쇼 OFF)* | `config.js.base` | 기본 정보 화면 |
...
3. **평일 주간 및 이른 아침 (07:00 ~ 08:00) ➔ 기본 화면 (`config.js.base`)**:
